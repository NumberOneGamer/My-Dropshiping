...
export const services = {
  products: {
    ...
    async getRelated(productId: string, categoryId?: string, limit = 4) {
      return safeQuery(async () => {
        ...
        return db.select().from(productsTable).where(and(...conditions)).orderBy(desc(productsTable.createdAt)).limit(limit);
      }, []);
    },
  },
  ...
  orders: {
    ...
    async getByUser(userId: string) {
      return safeQuery(async () => db.select().from(orders).where(eq(orders.userId, userId)).orderBy(desc(orders.createdAt)), []);
    },
